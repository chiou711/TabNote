...
public class DragNDropListView extends ListView {
	...
	public static interface OnItemDragNDropListener {
		...
		//cw
		public void onItemClick(DragNDropListView parent, View view, int position, long id);
		public void onItemLongClick(DragNDropListView parent, View view, int position,long id);
	}
	
	boolean mDragMode;
	boolean mClickMode; //cw
	boolean mNoStopDrag;
	...
	int mDragHandler = 0;
	int mClickHandler = 0; //cw
	
	ImageView mDragView;
	ImageView mClickView; //cw
	...
	public void setDragNDropAdapter(DragNDropAdapter adapter) {
		...
		mClickHandler = adapter.getClickHandler(); //cw
		setAdapter(adapter);
	}
...
//		if (mDragHandler == 0) return false;
	...
	//cw
	public boolean isClick(MotionEvent ev) {
//		if (mDragMode) return true;
//		if (mClickHandler == 0) return false;
//		if (mClickMode) return true;
		...
		int childposition = startposition - getFirstVisiblePosition();
		View parent = getChildAt(childposition);
		View handler = parent.findViewById(mClickHandler);
...
//		return true;
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		{
			mDragMode = true;
			System.out.println("mDragMode = true");
		}
		
		if (action == MotionEvent.ACTION_DOWN && isClick(ev)) 
		{
			mClickMode = true;
			System.out.println("mClickMode = true");
		}
		
//		if (!mDragMode) 
			if (!mDragMode && !mClickMode) //cw
			...
			case MotionEvent.ACTION_DOWN:
				if(mDragMode) //cw
				{
					...
					if (mStartPosition != INVALID_POSITION) {
	                    ...
	                    System.out.println("mStartPosition 1 = " + mStartPosition);
						startDrag(childPosition, y);
						System.out.println("mStartPosition 2 = " + mStartPosition);
						drag(0, y);
						System.out.println("MotionEvent.ACTION_DOWN & drag mode");//cw
					}
				}
				else if(mClickMode)
					System.out.println("MotionEvent.ACTION_DOWN & click mode");//cw
				break;
			case MotionEvent.ACTION_MOVE:
				if(mDragMode) //cw
				{
					drag(0, y);
					System.out.println("MotionEvent.ACTION_MOVE");//cw
					mClickMode = false;
					mNoStopDrag = false;
				}
				break;
...
//				if(!mDragMode)
				{
//					mStartPosition = pointToPosition(x, y);
					System.out.println("mStartPosition 3 = " + mStartPosition);
					int childPosition = mStartPosition - getFirstVisiblePosition();
					View item = getChildAt(childPosition);
					long id = getItemIdAtPosition(mStartPosition);
					if (mDragNDropListener != null)
			        	mDragNDropListener.onItemClick(this, item, mStartPosition, id);
					System.out.println("Lib ClickMode childPosition = " + childPosition);
					System.out.println("Lib ClickMode mStartPosition = " + mStartPosition);
					System.out.println("Lib ClickMode id = " + id);
					mNoStopDrag = true;
				}
	...
	private void startDrag(int childIndex, int y) {
		...
		System.out.println("childIndex = " + childIndex);//cw
		if (item == null) return;
...
//        dndAdapter.onItemClick(this, item, mStartPosition, id); //cw
